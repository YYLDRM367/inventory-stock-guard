...
type DbItem = { id: string; name: string; sku: string; cost: number };
type Line = { item_id: string; qty_ordered: number; unit_cost: number };
...
export function NewPOModal({ onClose, onCreated }: Props) {
  ...
  const [lines, setLines] = useState<Line[]>([{ item_id: "", qty_ordered: 1, unit_cost: 0 }]);

  useEffect(() => {
    Promise.all([fetch("/api/suppliers"), fetch("/api/items")]).then(async ([sr, ir]) => {
  ...
  function addLine() {
    setLines((l) => [...l, { item_id: "", qty_ordered: 1, unit_cost: 0 }]);
  }
    ...
    setLines((prev) =>
      prev.map((l, idx) => {
        ...
        if (field === "item_id") {
          ...
          if (it) updated.unit_cost = it.cost;
        }
        return updated;
      }),
    );
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label htmlFor="supplier_id">{lang === "tr" ? "Tedarikçi" : "Supplier"}</Label>
              <select
                id="supplier_id"
                name="supplier_id"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="number">{lang === "tr" ? "Sipariş no" : "PO number"}</Label>
              <Input id="number" name="number" placeholder={autoNumber} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="expected_at">{lang === "tr" ? "Tahmini teslim" : "Expected delivery"}</Label>
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              <Label>{lang === "tr" ? "Kalemler" : "Items"}</Label>
              <button
                type="button"
                onClick={addLine}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/30 text-left">
                    ...
                    <th className="label-mono w-28 px-3 py-2 font-medium text-muted-foreground">{lang === "tr" ? "Birim fiyat" : "Unit cost"}</th>
                    ...
                    <tr key={i} className="border-b border-border/60 last:border-0">
                      <td className="px-3 py-2">
                        <select
                          ...
                          className="w-full rounded-md border border-input bg-card px-2 py-1.5 text-[13px] text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
                        >
                      ...
                      <td className="px-3 py-2 text-right tnum text-[13px] font-semibold">
                        {formatUsd(line.qty_ordered * line.unit_cost)}
                      </td>
