...
- Crate's default is a warm **amber** (`oklch(70% 0.15 65)`). Convert the user's
  color to `oklch(...)` and update `--color-primary`, `--color-accent`,
  `--color-primary-foreground` and `--color-ring` in the `@theme` block of
  `app/globals.css`.
- Quick recipe: pick the hue (H) of their color and reuse the existing L/C, e.g.
  amber ≈ `oklch(70% 0.15 65)`, emerald ≈ `oklch(60% 0.14 155)`,
  blue ≈ `oklch(60% 0.16 255)`, rose ≈ `oklch(62% 0.2 15)`.
- The stock colors (`--color-stock-in` green, `--color-stock-out` red) are
  semantic — leave them unless the user insists.
